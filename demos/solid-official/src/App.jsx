...
export default function App(props) {
  ...
  return (
    <Switch fallback={<RenderOnce label="kit" multiplier={2} visible={true} onTrace={noTrace} />}>
      ...
      {/*
        THE FOURTH APPLICATION - the HACKER NEWS FRONT PAGE - and one of the
        SIX-LANE APPLICATIONS. This block used to call it "the FIRST route in this
        demo whose lane count is SIX" and it never was: S10 has carried no
        `unbuilt` entry in scripts/demo.mjs in any revision, and every stale
        position in this family counted from this sentence.
        S11 and S12 USED TO LOSE ANGULAR to its
        global-identifier ban and `frameless-app-fidelity-v1` T007 closed that, so
        all SIX CORPUS LANES now emit all three. S13 NEVER NEEDED THAT REPAIR, and
        that difference is the durable half: it names no global at all, because
        every relative
      ...
      */}
      ...
      `Date` stays refused. The position this line used to state instead counted
      from S13, which was never first - see the /hn block above.
      This comment used to add "UNLIKE S16 THE AXIS IT MEASURES IS ACTUALLY ON
      ...
      */}
      ...
      {/*
        ...
        `frameless-app-fidelity-v1` ruled a TWO-NAME allowlist - Promise and setTimeout,
        nothing else - and T007 landed it. packages/frameworks/angular/generated/S11.ts
        exists and THE ANGULAR LANE SERVES /todomvc-advanced: measured at HEAD by T014
        on a booted `ng serve`, 5,049 bytes of SSR body carrying `<app-root>`, "What
        needs to be done", "todoapp" and a LINKED
      ...
      */}
      ...
      {/*
        ...
        to be REFUSED outright. It is not, and it is no longer even partial: SIX lanes
        run it. This comment used to read "FOUR lanes run it, one emits and misbehaves,
        one refuses at emit", and BOTH of those exceptions are closed.
        ...
        `setTimeout`, and that lane could not NAME a global inside a transplanted body.
        `frameless-app-fidelity-v1` T007 landed the two-name allowlist and the route
        exists. MEASURED AT HEAD BY T014 on a booted `ng serve`: /codex answers with
        5,356 bytes of SSR body carrying `<app-root>`, "composer" five times and
        ...
        in any emitted vue SFC, which is now a permanent assertion in that lane's tests
