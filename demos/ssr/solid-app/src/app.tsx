...
type EmittedComponent = Component<Record<string, unknown>>;
...
export const routes = uiKitScenarios.map((scenario) => {
	...
	const path = paths[id];
	if (!component || !path) throw new Error(`Unknown Solid SSR scenario: ${scenario.id}`);
...
export function routeForPath(path: string) {
	...
	if (!route) throw new Error(`Unknown Solid SSR route: ${path}`);
	return route;
}
