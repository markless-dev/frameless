// Copies the contacts stylesheet into one lane's static-asset directory.
//
// SAME SHAPE AS `copy-board-css.mjs`, `copy-habit-css.mjs`, `copy-todomvc-css.mjs`
...
// digests.
//
// The file lands at `<asset-root>/contact-css/contacts.css`, so every lane serves it
// at the identical URL `/contact-css/contacts.css` and the six `/contacts` pages are
// like for like.
//
// ONE FILE, NOT A PAIR - BUT IT IS ONE HALF OF A PAIR AT LINK TIME. `contacts.css`
// is entirely this repository's own work (see ./contact-css/README.md - NOTHING
...
// `<asset-root>/shadcn-theme/tokens.css`. Every lane's `/contacts` route therefore
...
const source = resolve(here, 'contact-css');

const stylesheets = ['contacts.css'];

const assetRoot = process.argv[2];
if (assetRoot === undefined) {
	throw new Error(
		'copy-contact-css.mjs needs the lane-relative asset root, e.g. `node ../shared/copy-contact-css.mjs public`.',
	);
}

const target = resolve(process.cwd(), assetRoot, 'contact-css');
