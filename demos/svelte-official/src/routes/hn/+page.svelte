...
<!--
	...
	EVERY LANE now emits all three. NO NUMBER IS WRITTEN IN THAT SENTENCE ON
	PURPOSE: ruling 10's sweep is a JS lexer, so this `.svelte` file is invisible to
	`pnpm check:citations` and a count here would look guarded without being it.
	S13 NEVER NEEDED THAT REPAIR, and that difference is the durable half: it names
	no global anywhere, because every relative
