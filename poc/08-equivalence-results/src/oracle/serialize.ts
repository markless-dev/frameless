...
// Explicit and narrow: classes, styles, data-* and unknown attributes survive.
...
export class Observer {
  ...
  private id(node: Node) { let id = this.ids.get(node); if (!id) { id = this.nextId++; this.ids.set(node, id); } return id; }
  ...
  private serialize(node: Node): SerializedNode {
    ...
    const attributes = Array.from(el.attributes).filter((a) => !FRAMEWORK_ATTRIBUTE_ALLOWLIST.has(a.name))
  ...
  private path(host: HTMLElement, el: HTMLElement) { const parts: number[] = []; let node: Node = el; while (node !== host && node.parentNode) { parts.unshift(Array.from(node.parentNode.childNodes).indexOf(node)); node = node.parentNode; } return parts.join('.'); }
  private selection(el: HTMLElement): [number, number] | null { return el instanceof HTMLInputElement || el instanceof HTMLTextAreaElement ? [el.selectionStart ?? 0, el.selectionEnd ?? 0] : null; }
}
