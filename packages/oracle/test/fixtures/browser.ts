...
function observableStamp(host: HTMLElement): string {
	const controls = Array.from(
		host.querySelectorAll<HTMLInputElement | HTMLTextAreaElement | HTMLOptionElement>(
			'input,textarea,option',
		),
	).map((control) => ({
		value: control.value,
		checked: control instanceof HTMLInputElement ? control.checked : undefined,
		selected: control instanceof HTMLOptionElement ? control.selected : undefined,
	}));
	...
	return JSON.stringify({
		...
		focus: active
			? [
					active.getAttribute('data-edit') ?? active.getAttribute('data-action'),
...
export async function boundedBrowserQuiescence(
	host: HTMLElement,
