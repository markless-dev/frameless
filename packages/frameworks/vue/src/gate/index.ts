		...
		if (directive.name === 'model')
			violations.push(
				violation(
					file,
					'no-two-way-binding',
					'Emitted Vue source uses v-model. Worked example 12a rules this form DENIED on ITS OWN grounds, MEASURED against vue@3.5.40 - do not read it as worked example 3, which rules a different macro (defineEmits), and do not read it as denied at Gate 2, which it PASSES. G4 FAIL: the domain is every host renderHost() prints with a value/checked binding from attributesOf() plus a same-host event from eventAttribute(); re-enumerated over the eleven-scenario corpus it holds EIGHTEEN shipped instances and the sugar applies to SEVEN, because the other eleven handlers do strictly more than the assignment - they call props.onTrace(...), which is the e2e oracle observation channel, and v-model generates $event => ((x) = $event) and nothing else. THE ELEVENTH SCENARIO RE-ARGUED THIS GATE RATHER THAN MERELY RENUMBERING IT, AND IT STRENGTHENS THE DENIAL TWICE OVER: seven of eighteen is still a recognized subset - 39% against the 25% the tenth scenario left - so Gate 4 FAILs on exactly the criterion it failed on before, and the repair narrowing this entry recorded - handlers whose declared writes is exactly the bound node, whose reads is empty, and which carry no syncPolicy - has gone from ONE shipped instance (S2 event:0) to THREE (adding S10 event:2, S10 event:6) to SEVEN (adding S11 event:2, S11 event:5, S11 event:6, S11 event:10), each measured off the goldens by TWO INDEPENDENT DERIVATIONS THAT AGREE - one walking the emitted templates with @vue/compiler-sfc, one walking the compiler goldens handler ASTs. AND THE SECOND GROUND IS NEW IN KIND RATHER THAN IN COUNT: every applicable instance before S11 bound value, so this narrowing had only ever been exercised on TEXT INPUTS, and S11 event:6 is the FIRST v-model-shaped checked instance in the corpus - so the G3 unsoundness below is now reachable through event.currentTarget.checked as well as through event.currentTarget.value, and a matcher written against the .value shape alone would now be wrong on a shipped handler. A candidate sugar whose correct cases more than double while staying a minority of its domain, and which simultaneously crosses into a second bound property kind, has not moved toward totality; it has widened the surface on which the unchecked right-hand side would be wrong. G3 FAIL on the repair: StateWriteRecord (schema.ts:266) records operation "assign" and carries the right-hand side only as an AST, so draft = event.currentTarget.value and draft = otherEl.value are the SAME declared record, and separating them means matching the shape of an expression. G5 FAIL, four measured differences: the value stops being a vnode prop and the element LOSES the NEED_HYDRATION patch flag (40 PROPS|NEED_HYDRATION becomes 512 NEED_PATCH); vModelText.created attaches its own input/compositionstart/compositionend/change listeners and DROPS any input fired while el.composing is true; vModelText.mounted writes el.value unconditionally, on hydration too; and on a checkbox the SSR OUTPUT itself gains an Array.isArray(...) ? ssrLooseContain(...) branch the baseline does not have. G6 FAIL: no emitted artifact to regress. IR-1 and IR-2 are real gaps but they are NOT what denies this form, and IR-4 was never its blocker - v-model on a host element is not version-gated at all',
					line,
				),
			);
	}
