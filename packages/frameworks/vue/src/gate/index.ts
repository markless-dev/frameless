		...
		if (name === 'defineModel')
			violations.push(
				violation(
					file,
					'no-two-way-binding',
					'Emitted Vue source calls defineModel(). Worked example 12b rules it DENIED on ITS OWN grounds, MEASURED against vue@3.5.40. It is NOT worked example 3, which rules defineEmits, and it is NOT denied at Gate 2 - that prediction is REFUTED: useModel (runtime-core 3.5.40, useModel setter) reads the PARENT vnode props at runtime and falls back to a purely local value when the parent did not use v-model, so defineModel is the child module declaring itself and asks nothing of anyone. G4 FAIL: the domain is every PropDestructuringEntry propsDeclaration() prints into defineProps([...]); re-enumerated over the ten-scenario corpus it holds TWENTY-TWO printed entries spanning seven distinct prop names, and the sugar applies to ZERO of them, because its precondition is the component writing back to the prop. The repair narrowing "props the component writes back to" is NOT STATABLE: every prop entry in every golden shares ONE graph node, prop:props, declared writable=false with zero writes, so per-prop write-back has no channel in the IR at all. THE TENTH SCENARIO WAS RE-MEASURED AGAINST THIS CLAIM AND DID NOT DENT IT: S10 is a whole application rather than an axis probe, and it still declares exactly one printed entry (onTrace, a name the corpus already carried, which is why the distinct-name figure did not move) whose prop:props node is writable=false with zero writes. A real app was the strongest available chance for a written-back prop to appear, and none did - so ZERO is re-derived here, not carried forward. That is IR-1, and it is distinct from IR-8, which is a missing prop TYPE field. G5 FAIL: defineModel("x") compiles to props: mergeModels([...], { x: {}, xModifiers: {} }) plus emits: ["update:x"] plus a customRef local, so the module SILENTLY gains a prop the author never declared, gains an emits option, and changes every read site from a value to a ref - and xModifiers COLLIDES with a legal frameless prop of that name with ZERO diagnostics, because mergeModels falls to extend({}, normalizePropsOrEmits(a), normalizePropsOrEmits(b)) and the synthesized object wins. That is the Vue instance of worked example 4 Angular count/countChange, measured here rather than borrowed. G6 FAIL: no emitted artifact to regress; this check pins the DENIAL, not the sugar. IR-4 was never the blocker - four gates FAIL at the version this repo ships, and FAIL outranks DEFERRED',
					line,
				),
			);
