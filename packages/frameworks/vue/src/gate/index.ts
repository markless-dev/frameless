...
export const VUE_ESLINT_TIERS_EXCLUDED: readonly ExcludedEslintTier[] = [
	...
	},
	{
		...
		reason:
			'The "Priority C: recommended, minimising arbitrary choices and cognitive overhead" tier, and it is a SUPERSET of flat/strongly-recommended - so excluding it follows from the row above rather than from anything it adds on its own. FOR THE FIRST TIME SINCE THIS ROW WAS WRITTEN IT NO LONGER REPORTS THE SAME IDS AS THE ROW ABOVE, and that is a measurement rather than a maintenance edit: the sentence this row used to carry - that the eight rules this tier adds (vue/attributes-order, vue/block-order, vue/no-lone-template, vue/no-multiple-slot-args, vue/no-required-prop-with-default, vue/no-v-html, vue/order-in-components, vue/this-in-template) are all silent on emitted output - WAS TRUE FOR SIXTEEN SCENARIOS AND IS NOW FALSE FOR ONE OF THEM. vue/attributes-order fires EIGHTEEN TIMES, all of them in S17 and all of them the identical complaint: "Attribute id should go before class". S17 is the FORMS card and the first module here where bound controls also need ids, because a <label for> points at each of them; S13 footer search field is the only earlier host with an id and it happens to print id first, which is why sixteen scenarios of silence measured nothing about this rule. THE EXCLUSION IS UNCHANGED AND SO IS ITS GROUND. This rule is pure attribute ORDER: it moves no rendered byte, it is not a correctness rule, and the emitter prints attributes in AUTHORED order, so it is a claim about the fixture rather than about the emitter. It was deliberately NOT silenced by reordering the fixture, because letting a tier this lane does not apply dictate authoring order is the failure mode this list exists to prevent - and because five other lanes emit the same source and none of them holds an opinion about it. The two content-newline rules in the row above still decide the exclusion on their own. Recorded so that a later rule arriving in this tier and firing is a red test here rather than an unexamined exclusion - which is exactly what happened.',
		firesOnCorpus: [
			// NOT INHERITED FROM THE ROW ABOVE - this id is what this tier ADDS, and
			// S17 is the first scenario in sixteen able to make it fire. See the
			// reason string; it is attribute ORDER and it moves no rendered byte.
		...
		if (directive.name === 'model')
			violations.push(
				violation(
					file,
					'no-two-way-binding',
					'Emitted Vue source uses v-model. Worked example 12a rules this form DENIED on ITS OWN grounds, MEASURED against vue@3.5.40 - do not read it as worked example 3, which rules a different macro (defineEmits), and do not read it as denied at Gate 2, which it PASSES. G4 FAIL: the domain is every host renderHost() prints with a value/checked binding from attributesOf() plus a same-host event from eventAttribute(); re-enumerated over the sixteen-scenario corpus it holds THIRTY-EIGHT shipped instances and the sugar applies to NINE, because the other twenty-nine handlers do strictly more than the assignment - they call props.onTrace(...), which is the e2e oracle observation channel, and v-model generates $event => ((x) = $event) and nothing else. THE ELEVENTH, TWELFTH, THIRTEENTH, FIFTEENTH AND SIXTEENTH SCENARIOS EACH RE-ARGUED THIS GATE RATHER THAN MERELY RENUMBERING IT. COUNTS FIRST: nine of THIRTY-EIGHT is still a recognized subset - 24%, and it is now the LOWEST RATIO THIS ENTRY HAS EVER RECORDED, below the 25% the tenth scenario left, against the 45% the thirteenth left, the 42% the twelfth and the 39% the eleventh - so Gate 4 FAILs on exactly the criterion it failed on before, and the repair narrowing this entry recorded - handlers whose declared writes is exactly the bound node, whose reads is empty, and which carry no syncPolicy - has gone from ONE shipped instance (S2 event:0) to THREE (adding S10 event:2, S10 event:6) to SEVEN (adding S11 event:2, S11 event:5, S11 event:6, S11 event:10) to EIGHT (adding S12 event:4) to NINE (adding S13 event:26), each measured off the goldens by TWO INDEPENDENT DERIVATIONS THAT AGREE - one walking the emitted templates with @vue/compiler-sfc, one walking the compiler goldens handler ASTs. S11\'S GROUND WAS A SECOND BOUND PROPERTY KIND: every applicable instance before it bound value, so the narrowing had only ever been exercised on TEXT INPUTS, and S11 event:6 is the FIRST v-model-shaped checked instance in the corpus - so the G3 unsoundness below is reachable through event.currentTarget.checked as well as through event.currentTarget.value, and a matcher written against the .value shape alone would be wrong on a shipped handler. S12\'S GROUND IS A SECOND TAG, AND IT IS THE STRONGER OF THE TWO because it moves G5 rather than only G3: every one of the previous eighteen instances was an <input>, and S12\'s composer is the corpus\'s first value-bound <textarea> - S7 ships a textarea but binds data-notes, not value. The domain therefore spans two tags as well as two bound property kinds, and the fifth G5 difference listed below EXISTS ONLY ON THE NEW TAG. A candidate sugar whose correct cases have grown ninefold while staying a minority of its domain, and which has now crossed into a second bound property kind AND a second element type, has not moved toward totality; it has widened the surface on which the unchecked right-hand side would be wrong, and widened it onto a tag whose SSR behaviour differs in kind. G3 FAIL on the repair: StateWriteRecord (schema.ts:266) records operation "assign" and carries the right-hand side only as an AST, so draft = event.currentTarget.value and draft = otherEl.value are the SAME declared record, and separating them means matching the shape of an expression. G5 FAIL, five measured differences, the last of them measured on S12\'s new tag rather than carried over from the input case: the value stops being a vnode prop and the element LOSES the NEED_HYDRATION patch flag (40 PROPS|NEED_HYDRATION becomes 512 NEED_PATCH), reproduced on <textarea> as well as on <input>; vModelText.created attaches its own input/compositionstart/compositionend/change listeners and DROPS any input fired while el.composing is true; vModelText.mounted writes el.value unconditionally, on hydration too; on a checkbox the SSR OUTPUT itself gains an Array.isArray(...) ? ssrLooseContain(...) branch the baseline does not have; and ON A TEXTAREA THE TWO FORMS SSR-RENDER FROM DIFFERENT SOURCES ENTIRELY - the baseline emits ssrRenderAttrs(_temp0 = mergeProps({ value: _ctx.draft }, _attrs)) and then interpolates ("value" in _temp0) ? _temp0.value : "", so a parent-supplied value participates, while v-model emits ssrRenderAttrs(_attrs) and interpolates ssrInterpolate(_ctx.draft) directly, so it does not. That is a fallthrough-attribute semantic the input case cannot exhibit at all, because an input has no text child. S13\'S GROUND IS THE ONE NOBODY LOOKED FOR, AND IT IS A NEGATIVE: THE FOURTH WHOLE APPLICATION PRODUCED NO NEW SHAPE AT ALL. The HN front page contributes exactly ONE host to this domain - its footer search field - and that host is a plain <input> with a value bind and an input handler, structurally identical to S10\'s new-todo field. It moves the COUNT (nineteen -> twenty), the RATIO (42% -> 45%) and the narrowing\'s reach (eight -> nine), and it moves NEITHER the bound-property-kind span (still value and checked) NOR the tag span (still <input> and <textarea>), so not one of the five G5 differences below changes. THAT ABSENCE IS THE DATUM: the eleventh scenario added a property kind, the twelfth added a tag, and the thirteenth - a whole application with sixty-two hosts, twenty-seven recorded events and its own form - added neither, which is the first evidence in this corpus that the G5 list is COMPLETE rather than merely un-probed. THE ONE THING S13 DOES CARRY THAT NOTHING ELSE HERE DOES is an id attribute on the bound host, because a <label for> points at it; it is recorded because it is the first, and it is recorded as IRRELEVANT because vModelText is selected on type and not on any other attribute, so an id changes no lowering. AND THE STATED REASON WAS RE-DERIVED, NOT CARRIED: all ELEVEN handlers outside the sugar still call props.onTrace(...) - the outside count did not move, because S13\'s single new instance landed INSIDE the reach. S15\'S GROUND IS A SECOND NEGATIVE AND IT IS STRICTLY STRONGER THAN S13\'S, BECAUSE THIS TIME NOT EVEN THE COUNT MOVED. The habit tracker is the FIFTH whole application and the LARGEST TEMPLATE IN THE CORPUS at EIGHTY-ONE hosts - nineteen more than S13 - and it contributes ZERO hosts to this domain. It is the first application here WITH NO FORM CONTROL AT ALL: derived independently with @vue/compiler-sfc over the emitted S15.vue, it carries SEVEN hosts with an on-directive and NOT ONE value or checked bind, because every interaction on the page is a <button> or <a> click. So the instance count holds at twenty, the applicable count at nine, the ratio at 45%, the tag span at <input> and <textarea>, and the bound-property-kind span at value and checked. S13 was a null result that still moved the count from nineteen; S15 moves NOTHING but the scenario count, which is what a domain looks like once it has stopped growing - and it is the second consecutive whole application to add no new G5 shape, which is why the completeness reading S13 opened is now supported by two independent applications rather than one. IT ALSO CARRIES THE STRONGEST AVAILABLE SHAPE THIS SUGAR COULD HAVE WANTED AND STILL DOES NOT QUALIFY: its one real interaction fans a single click out to EIGHT derived observables, so if any authoring in this repo were going to want two-way binding it would be this one, and what it needs instead is one-way derivation from a single cell. S16\'S GROUND IS A THIRD CONSECUTIVE NEGATIVE, AND IT IS THE ONE THAT WAS SUPPOSED TO BE DIFFERENT. The task board is the SIXTH whole application and it TAKES THE LARGEST-TEMPLATE TITLE OFF S15 at EIGHTY-NINE hosts, and it is the DRAG scenario - the first application in this corpus whose defining question is an INPUT DEVICE rather than a data shape. Derived independently with @vue/compiler-sfc over the emitted S16.vue: TWELVE hosts carry an on-directive, more than any scenario here except S13\'s twenty-five, and NOT ONE of them carries a value or checked bind, so it contributes ZERO instances. The instance count holds at twenty, the applicable count at nine, the ratio at 45%, the tag span at <input> and <textarea>, and the bound-property-kind span at value and checked - the same five figures S15 held. THREE CONSECUTIVE APPLICATIONS HAVE NOW ADDED NO G5 SHAPE, and the third did it while doubling the on-directive host count of the second, which separates "this domain has stopped growing" from "these two apps happened to have no forms": a page with twelve evented hosts and no bound value is positive evidence that the corpus\'s interaction surface is CLICKS, not two-way fields. AND S16 CARRIES THE ONE THING THE DOMAIN STILL HAS NO INSTANCE OF, WHICH IS WHY IT IS RECORDED AS ABSENT RATHER THAN AS SILENCE: it is the first module here that WANTED a drag interaction and could not have one, so the hosts that would have carried it - its single <ul> and single <li> - carry nothing bindable at all. S17\'S GROUND ENDS THE RUN OF NEGATIVES, AND IT MOVES THE ONE FIGURE THREE SCENARIOS OF SILENCE HAD BEEN READ AS CLOSED. The contacts page is the SEVENTH whole application, the FORMS card, and the corpus\'s largest template at TWO HUNDRED AND TWELVE hosts - 2.4x S16\'s eighty-nine. Derived independently with @vue/compiler-sfc over the emitted S17.vue: THIRTY-ONE hosts carry an on-directive, more than any scenario in this corpus including S13\'s twenty-six, and EIGHTEEN of them also carry a value or checked bind. So instances go twenty -> THIRTY-EIGHT in one scenario - a bigger single move than every scenario since S7 combined - and the sugar\'s reach DOES NOT MOVE AT ALL, holding at nine, because all eighteen new handlers call props.onTrace(...). THE COMPLETENESS READING S13 OPENED AND S15/S16 SUPPORTED IS THEREFORE HALF REFUTED AND HALF CONFIRMED, and the split is the datum. REFUTED: the domain had not stopped growing; it had run out of forms to grow on, and three consecutive click-only applications cannot tell those apart - which is exactly the objection S16\'s row raised against S15\'s and then answered in the wrong direction. CONFIRMED: the SHAPES did not grow in proportion. The bound-property-kind span still holds at value and checked after eighteen new instances across thirteen distinct control types, so no third kind exists to find. THE TAG SPAN DOES MOVE, from two to THREE: <select> joins <input> and <textarea>, and it is not a cosmetic addition, because v-model on a <select> is vModelSelect and not vModelText - a different runtime directive that sets el.value in a mounted hook, reads el.options and el.selectedIndex, carries its own deep/number/trim modifier handling and, on a multiple select, produces an ARRAY where the baseline binding produces a string. S7 ships two <select>s and binds data-size on one of them, never value, so this corpus has never had an instance of that directive before. A candidate sugar whose domain has nearly DOUBLED while its correct cases held flat has moved further from totality, not closer, and it has moved onto a third element type whose runtime directive is a different one again. AND THE ONE THING S17 CARRIES THAT IS OUTSIDE THE DOMAIN IS RECORDED BECAUSE IT IS THE FIRST NEGATIVE CONTROL THIS ENTRY HAS EVER HAD: its five <option value={row.id}> hosts carry a value bind and NO on-directive, so they are correctly excluded by the second half of the domain definition rather than by accident of nothing matching. S7 carries three of the same shape. Before S17, every bound host in this corpus also had an event, so the \'plus a same-host event\' clause had never actually excluded anything. G6 FAIL: no emitted artifact to regress. IR-1 and IR-2 are real gaps but they are NOT what denies this form, and IR-4 was never its blocker - v-model on a host element is not version-gated at all',
					line,
				),
			);
	}
		...
		if (name === 'defineModel')
			violations.push(
				violation(
					file,
					'no-two-way-binding',
					'Emitted Vue source calls defineModel(). Worked example 12b rules it DENIED on ITS OWN grounds, MEASURED against vue@3.5.40. It is NOT worked example 3, which rules defineEmits, and it is NOT denied at Gate 2 - that prediction is REFUTED: useModel (runtime-core 3.5.40, useModel setter) reads the PARENT vnode props at runtime and falls back to a purely local value when the parent did not use v-model, so defineModel is the child module declaring itself and asks nothing of anyone. G4 FAIL: the domain is every PropDestructuringEntry propsDeclaration() prints into defineProps([...]); re-enumerated over the sixteen-scenario corpus it holds TWENTY-EIGHT printed entries spanning seven distinct prop names, and the sugar applies to ZERO of them, because its precondition is the component writing back to the prop. The repair narrowing "props the component writes back to" is NOT STATABLE: every prop entry in every golden shares ONE graph node, prop:props, declared writable=false with zero writes, so per-prop write-back has no channel in the IR at all. THE ELEVENTH, TWELFTH, THIRTEENTH, FIFTEENTH AND SIXTEENTH SCENARIOS WERE EACH RE-MEASURED AGAINST THIS CLAIM AND NONE DENTED IT, AND EACH WAS A STRICTLY STRONGER TEST THAN THE ONE BEFORE. S11 is TodoMVC ADVANCED - a second whole application and the first whose defining mechanism is ASYNCHRONOUS, with nineteen recorded events of which two suspend across an await. If any authoring were going to write back through a prop it would be a handler resuming from a remote answer, and none does. S12 is the CODEX CLONE, a THIRD whole application, the largest TEMPLATE in the corpus at fifty-three hosts, and the first module here to STREAM: one of its nine recorded events suspends THREE TIMES in a single handler and writes the message list after each resume. A streaming child pushing partial answers upward is the single most natural shape a written-back prop could take - it is what `defineModel` exists for - and this corpus\'s strongest available instance of it still declares exactly one printed entry, onTrace, a name already in the corpus, which is why the entry count moved from twenty-three to twenty-four while the distinct-name figure held at seven. Its prop graph is one node, prop:props, writable=false with zero writes. S13 is the HACKER NEWS FRONT PAGE, a FOURTH whole application and the FIRST one this repo emits in all SIX lanes, with sixty-two template hosts and twenty-seven recorded events; it is also the first module here whose defining problem is that IT HAS NO DOOR - fetch-on-render is unreachable in every lane, so every byte it renders is seeded in the component. A component that cannot ask its parent for data is the shape most likely to reach for a prop channel in the other direction, and it does not: one printed entry, onTrace, a name the corpus already had, so the entry count moved twenty-four -> twenty-five while the distinct-name figure held at seven for the fifth consecutive time. Its prop graph is one node, prop:props, writable=false with zero writes. S15 is the HABIT TRACKER, a FIFTH whole application, the corpus\'s LARGEST TEMPLATE at eighty-one hosts, and the first one whose defining mechanism is SYNCHRONOUS DERIVED FAN-OUT: a single click on one of six habits moves EIGHT observables across three separate subtrees, all of them derived from ONE state cell through computed values and class/hidden bindings. That is the shape a two-way channel would most plausibly be reached for - a child that has to keep many places in step - and the direction it actually needs is the opposite one, DOWN from a single cell rather than UP through a prop. It declares one printed entry, onTrace, a name the corpus already had, so the entry count moved twenty-five -> twenty-six while the distinct-name figure held at seven for the SIXTH consecutive time. Its prop graph is one node, prop:props, writable=false with zero writes. S16 is the TASK BOARD, a SIXTH whole application, the corpus\'s largest template at eighty-nine hosts, and the first one whose defining question is an INPUT DEVICE rather than a data shape: it is the DRAG scenario, and it ships without drag. That absence is the strongest test this entry has had, and in a direction none of the five before it could reach - a component that cannot express an interaction it needs has an obvious escape, which is to push the missing behaviour UP to a parent through a written-back prop, and this module does not take it. It declares one printed entry, onTrace, a name the corpus already had, so the entry count moved twenty-six -> twenty-seven while the distinct-name figure held at seven for the SEVENTH consecutive time. Its prop graph is one node, prop:props, writable=false with zero writes. S17 is CONTACTS, a SEVENTH whole application, the corpus\'s largest template at two hundred and twelve hosts, and THE FORMS CARD: thirteen distinct control kinds, eighteen hosts carrying a value or checked bind, thirty-one evented hosts and thirty-five state writes, all of them into component-local cells. IT IS THE STRONGEST TEST THIS ENTRY HAS HAD AND THE FIRST ONE AIMED AT ITS ACTUAL PRECONDITION. defineModel exists so that a child owning an EDITABLE VALUE can hand that value back to its parent, and a new-contact form owning fourteen editable cells is the single most natural shape for that in this whole corpus - far more natural than a stream or a fan-out, which is what the six rows above had to reach for. It still declares exactly ONE printed entry, onTrace, a name the corpus already had, so the entry count moved twenty-seven -> TWENTY-EIGHT while the distinct-name figure held at seven for the EIGHTH consecutive time. Its prop graph is one node, prop:props, writable=false with zero writes. SEVEN whole applications, an async axis, a three-chunk stream, a doorless page, a fan-out page, an application with an UNAUTHORABLE INTERACTION and now a FOURTEEN-CELL FORM have each been the strongest available chance for a written-back prop to appear, and none produced one - so ZERO is re-derived here for the EIGHTH time, not carried forward. That is IR-1, and it is distinct from IR-8, which is a missing prop TYPE field. G5 FAIL: defineModel("x") compiles to props: mergeModels([...], { x: {}, xModifiers: {} }) plus emits: ["update:x"] plus a customRef local, so the module SILENTLY gains a prop the author never declared, gains an emits option, and changes every read site from a value to a ref - and xModifiers COLLIDES with a legal frameless prop of that name with ZERO diagnostics, because mergeModels falls to extend({}, normalizePropsOrEmits(a), normalizePropsOrEmits(b)) and the synthesized object wins. That is the Vue instance of worked example 4 Angular count/countChange, measured here rather than borrowed. G6 FAIL: no emitted artifact to regress; this check pins the DENIAL, not the sugar. IR-4 was never the blocker - four gates FAIL at the version this repo ships, and FAIL outranks DEFERRED',
					line,
				),
			);
