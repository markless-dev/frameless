 ...
 * `self`, and this emitter refuses it by name: a `.svelte` file declares exactly
 * one component, and the only same-file alternative - a snippet - cannot own the
 * `state()` cells every instance of a recursive thread needs.
 *
 * THIS IS NOT A VERDICT ON RECURSION IN THIS LANE, and the difference was
 * MEASURED rather than assumed. Spelled the way Svelte spells recursion
 * NATIVELY - the module importing ITSELF under an alias - THIS EMITTER TAKES IT
 * and prints `import Self from './comment.svelte'` inside `comment.svelte`.
 ...
 *     module built from `s14-hn-item.tsrx` would import `./s14-hn-item.svelte`
 *     while the artifact on disk is `generated/S14.svelte`;
...
export const SVELTE_UNBUILT_SCENARIOS: readonly UnbuiltScenario[] = [
	{
		golden: 's14-hn-item.json',
		emitted: 'S14.svelte',
		refusalContains:
			'Svelte emitter has no lowering for a same-module component reference (HnItem)',
		reason:
			"S14's `HnItem` names ITSELF in its own template, and a .svelte file declares exactly one component while a snippet cannot own state or a lifecycle. `refusalContains` stops before the trailing clause so the row does not go red if that clause is reworded. THE NATIVE SVELTE SPELLING - the module importing itself under an alias - IS ACCEPTED BY THIS EMITTER and refused by `resolveModuleSet` instead (`Component-reference cycle`), and would in any case import `./s14-hn-item.svelte` while the artifact on disk is `generated/S14.svelte`.",
	},
];

const UNBUILT_GOLDENS = new Set(SVELTE_UNBUILT_SCENARIOS.map((entry) => entry.golden));
const UNBUILT_EMITTED = new Set(SVELTE_UNBUILT_SCENARIOS.map((entry) => entry.emitted));
 ...
 * `S14.svelte`, a `generated/S14.svelte`, or an absolute path, because the four
