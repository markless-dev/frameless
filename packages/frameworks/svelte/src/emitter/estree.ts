...
/**
 * Untyped ESTree node bags. The Svelte emitter builds output ASTs from the
 * IR's own `SerializableAstNode`s, which carry an open `[key: string]` shape,
 * so a structural type here would be fiction. Same choice as the Qwik and Solid
 * emitters.
 */
 ...
 * formatter may be run over a `.svelte` file (T003 stop_if - `prettier`,
 * `prettier-plugin-svelte` and `oxfmt` are all out of scope here). One space
 * per level is generated and converted 1:1 into tabs by `toTabs` below, which
 * is exact rather than a heuristic: a single leading space can only be
 ...
 * continuation line's own leading spaces are payload, not indentation, and
 * would be silently corrupted. This walks the output AST and refuses instead.
 */
function assertNoMultilineLiterals(program: Node): void {
	walk(program, (record) => {
		if (record.type === 'TemplateElement' && String(record.value?.raw ?? '').includes('\n'))
			throw new Error(
				'Svelte emitter cannot indent a template literal that spans a line break',
			);
		if (
			record.type === 'Literal' &&
			...
			throw new Error('Svelte emitter cannot indent a string literal that spans a line break');
	});
}
...
const EXPRESSION_PROBE = '__frameless_svelte_expression__';

/**
 * Print one expression with no statement punctuation.
 *
 * Generated through a declarator rather than an expression statement so an
 * arrow function is never parenthesised and never picks up a trailing `;`. The
 * fixed prefix/suffix are asserted before they are stripped, so a codegen
 * change surfaces as a throw instead of a silently truncated handler.
 */
export function printExpression(value: Expression): string {
	...
	if (!printed.startsWith(prefix) || !printed.endsWith(';'))
		throw new Error(
			`Svelte emitter could not isolate a printed expression from ${JSON.stringify(printed.slice(0, 60))}`,
		);
