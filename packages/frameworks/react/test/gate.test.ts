...
describe('React dossier gate', () => {
	...
	test('discovers, parses, and accepts every checked-in generated component', async () => {
		...
		for (const file of result.files) {
			expect(await readFile(file, 'utf8')).toContain('@generated by @frameless/react');
		}
	});
	...
	test.each([
		...
		['controlled input', valid.replace('<section>', '<section><input value={value} />').replace('</section>', '</section></section>'), 'controlled-input'],
		['onInput', valid.replace('<section>', '<section><input value={value} onInput={() => {}} />').replace('</section>', '</section></section>'), 'on-input'],
	...
	test('a newly added generated file is discovered and cannot bypass the gate', async () => {
		const root = await mkdtemp(resolve(tmpdir(), 'frameless-react-gate-'));
