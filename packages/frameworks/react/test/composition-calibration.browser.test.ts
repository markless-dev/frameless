...
import {
	makeReactCompositionMutant,
	ReactContainerScopeReference,
	ReactPageScopeReference,
	reactCompositionReferences,
	resetReactPageScopeReference,
...
describe('React composition oracle calibration', () => {
	for (const scenario of compositionScenarios) {
		test(`${scenario.id}: handwritten reference satisfies every expectation`, async () => {
			...
			const trace = await runComposition(
				reactCompositionReferences[scenario.id],
				scenario.id,
			);
	...
	test('F5: container scope isolates mounts and page scope shares them', async () => {
		const containerAdapter = createReactAdapter(ReactContainerScopeReference);
		...
		resetReactPageScopeReference();
		const pageAdapter = createReactAdapter(ReactPageScopeReference);
