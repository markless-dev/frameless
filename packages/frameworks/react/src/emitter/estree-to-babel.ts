...
export function fromEstree(input: SerializableAstNode | null | undefined): t.Node | null {
	...
	switch (node.type) {
		...
		case 'ArrayExpression':
			return t.arrayExpression(node.elements.map(fromEstree) as Array<t.Expression | t.SpreadElement | null>);
		case 'ObjectExpression':
			return t.objectExpression(node.properties.map(fromEstree) as Array<t.ObjectMethod | t.ObjectProperty | t.SpreadElement>);
		case 'Property':
		...
		case 'CallExpression':
			return t.callExpression(
				...
				node.arguments.map(fromEstree) as Array<t.Expression | t.SpreadElement | t.ArgumentPlaceholder>,
			);
