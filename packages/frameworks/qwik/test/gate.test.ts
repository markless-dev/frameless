...
describe('Qwik v2 dossier gate', () => {
	...
	/**
	 * THE SECOND OUTPUT DIRECTORY, WHICH THIS GATE HAD NEVER LOOKED AT.
	 *
	 * `frameless-app-axes-v1` T015 measured every lane against every generation
	 * tier and found the standing corpus of FOUR lanes - qwik, svelte, vue and
	 * angular - was `generated/` ONLY, while `generated-composition/` shipped
	 * committed artifacts no policy had ever been pointed at. React and Solid
	 * already carried a row like this one; these four did not.
	 *
	 * QWIK IS THE ONE LANE WHERE CLOSING IT COSTS NOTHING: pointed at this
	 * directory the gate reports ZERO violations and ZERO unevaluated policies,
	 * measured both with and without the fixture artifacts. Svelte, Vue and
	 * Angular each draw real `baseline-form-inventory` violations there, and those
	 * are pinned as DEBT in their own corpora rather than closed here, because
	 * admitting the forms needs a ruling.
	 *
	 * The file list is DERIVED from `compositionFixtures` - the same source the
	 * regeneration script uses - so a new fixture joins this row with no edit, and
	 * a fixture whose artifact went missing is red. The row below watches that
	 * comparison actually go red in both directions.
	 */
	test('discovers and gates every generated composition module', async () => {
		...
		const entries = await Promise.all(
			compositionFixtures.map(async (name) => ({
				file: `generated-composition/${name}${COMPOSITION_EXTENSION}`,
				source: await readFile(
					resolve(packageRoot, `generated-composition/${name}${COMPOSITION_EXTENSION}`),
					'utf8',
				),
				artifact: await buildEnrichedIr({
					filename: `test/composition-fixtures/${name}.tsrx`,
					source: await readFile(
						resolve(packageRoot, `test/composition-fixtures/${name}.tsrx`),
						'utf8',
					),
				}),
			})),
		...
		expect(result.violations, JSON.stringify(result.violations, null, 2)).toEqual([]);
		// WITH the artifact there is nothing left unevaluated, which is the half the
		// `generated/` row above cannot assert - it reads sources off disk with no
		// artifact, so `persistence-render-lowering` is permanently unevaluated there.
		expect(result.unevaluated).toEqual([]);
		for (const file of result.files)
			expect(await readFile(resolve(packageRoot, file), 'utf8')).toContain(
				'@generated by @frameless/qwik',
	...
	/**
	 * CALIBRATION for the row above. `[]` violations over a directory is exactly
	 * what a gate that never opened the directory reports, so the discovery half is
	 * driven against a throwaway root in both directions, and the checking half is
	 * shown to reject a planted mutant carrying the construct this lane refuses.
	 */
	test('CALIBRATION: the composition corpus goes red on a missing file, an extra file, and a mutant', async () => {
		const expected = compositionFixtures
			.map((name) => `generated-composition/${name}${COMPOSITION_EXTENSION}`)
			.sort();
		const root = await realpath(
			await mkdtemp(resolve(tmpdir(), 'frameless-qwik-composition-')),
		);
		try {
			await mkdir(resolve(root, 'generated-composition'));
			const stub = '// composition calibration\n';
			for (const file of expected.slice(0, -1)) await writeFile(resolve(root, file), stub);
			expect(
				await discoverGeneratedFiles({ cwd: root, directory: 'generated-composition' }),
			).not.toEqual(expected);
			await writeFile(resolve(root, expected.at(-1)!), stub);
			expect(
				await discoverGeneratedFiles({ cwd: root, directory: 'generated-composition' }),
			).toEqual(expected);
			await writeFile(resolve(root, 'generated-composition/C99-extra.tsx'), stub);
			expect(
				await discoverGeneratedFiles({ cwd: root, directory: 'generated-composition' }),
			).not.toEqual(expected);
		} finally {
		...
		// And the CHECKING half really can reject a file living under this
		// directory - otherwise the clean [] above would be indistinguishable from a
		// policy set that only ever fires on paths starting `generated/`.
		const source = await readFile(
			resolve(packageRoot, `generated-composition/M1-panel${COMPOSITION_EXTENSION}`),
			'utf8',
		);
		const mutant = mutate(
			mutate(source, 'Slot, component$ }', 'Slot, component$, useVisibleTask$ }'),
			'\treturn (',
			'\tuseVisibleTask$(() => {});\n\treturn (',
		);
		const mutated = await checkSources([
			{ file: `generated-composition/M1-panel${COMPOSITION_EXTENSION}`, source: mutant },
		]);
		expect(mutated.violations.map((entry) => entry.policy)).toContain('no-visible-task');
	});
