...
describe('Solid dossier gate', async () => {
	...
	test('discovers and accepts every checked-in generated component', async () => {
		...
		// NOT `[]` ANY MORE - AND READ THE MESSAGES, BECAUSE THEY ARE THE FINDING.
		//
		...
		// so the authored `onDragStart` / `onDragOver` / `onDragEnd` arrive at
		// every emitter already flattened. This lane prints `onDragover` and
		// `solid/event-handlers` says it "SHOULD BE RENAMED ... FOR READABILITY".
		//
		// THE REACT LANE'S GATE REPORTS THE SAME THREE SPELLINGS AS "Unknown
		// property". Two linters over one emitted string, and the difference in
		// wording is the difference in the defect: solid delegates by the real DOM
		// event name so the binding WORKS and only reads badly, while react-dom
		// matches a camelCase prop table so the binding never fires. THIS IS A
		// THIRD INDEPENDENT INSTRUMENT AGREEING THAT DEFECTS.md 15 IS REACT-ONLY,
		// after the six-lane emitter probe and the driven browser - and it is the
		// first one that is STATIC, so it runs on every `pnpm test`.
		//
		// The list is EXACT. A fourth violation, or one of these on another file or
		// policy, still fails. The rule is not disabled and the CALIBRATION below
		// proves it still reports on demand.
		const DEFECT_15_READABILITY_ONLY = [
			{
				dossierRef: 'T003 ruling 10',
				file: 'generated/S16.tsx',
				policy: 'eslint:solid/event-handlers',
				message: 'The onDragover prop should be renamed to onDragOver for readability.',
			},
			{
				dossierRef: 'T003 ruling 10',
				file: 'generated/S16.tsx',
				policy: 'eslint:solid/event-handlers',
				message: 'The onDragstart prop should be renamed to onDragStart for readability.',
			},
			{
				dossierRef: 'T003 ruling 10',
				file: 'generated/S16.tsx',
				policy: 'eslint:solid/event-handlers',
				message: 'The onDragend prop should be renamed to onDragEnd for readability.',
			},
		];
		// `line` is dropped for the reason the react lane's twin records: a line
		// ordinal in an expectation rots on the next prose edit, and this repo
		// measured that rot and built `scripts/check-citations.mjs` for it.
		expect(
			result.violations.map(({ line: _line, ...rest }) => rest),
			JSON.stringify(result.violations, null, 2),
		).toEqual(DEFECT_15_READABILITY_ONLY);
	...
	/**
	 * CALIBRATION FOR THE THREE RECORDED VIOLATIONS ABOVE, driven through the same
	 * `checkSources` entry point in both directions. ARM B is the one that
	 * matters: solid's own spelling is silent, so the rule answers to the SPELLING
	 * rather than to the presence of a drag handler.
	 */
	test('CALIBRATION: solid/event-handlers still fires, and only on the flattened spelling', async () => {
		const page = (attribute: string) =>
			`// @generated by @frameless/solid; do not edit.\n` +
			`export function Mutant() {\n` +
			`\treturn <ul ${attribute}={() => {}} />;\n` +
			`}\n`;
		expect(await policies(page('onDragover'))).toContain('eslint:solid/event-handlers');
		expect(await policies(page('onDragOver'))).not.toContain('eslint:solid/event-handlers');
		expect(await policies(page('onPointerdown'))).toContain('eslint:solid/event-handlers');
	});
	 ...
	 * lane already gated the first two (the row above), and
	 * `generated-persistence/P1.tsx` IS gated with its artifact, in
	 * `test/emitter.test.ts`. But it is gated BY NAME: nothing in this package ever
	 * asked the directory what it contains, so a SECOND persisted artifact would
	 * ship with no policy pointed at it and every row in the repo would stay green.
	 *
	 * This closes the discovery half only, and says so: the artifact-bearing check
	 * stays where it is, next to the emitter run that produces the record. React
	 * carries the identical row, named here on purpose - T015's whole method was
	 * that a measurement on one lane is not a measurement until a second lane runs
	 * the same instrument.
	 */
	test('discovers the whole generated-persistence tier, and names what source-only cannot check', async () => {
		...
		// must not appear without someone deciding it should.
		...
		// artifact-required policy is UNEVALUATED here, not passing - DERIVED from
		// the policy list so a new one joins without an edit.
