...
export function KeyedTodo(props) {
  const [todos, setTodos] = createStore(untrack(() => props.seed.map(todo => ({
    ...todo
  }))));
  ...
  return <section data-scenario="s2"><p data-count="complete">{complete()}/{todos.length}</p><input data-action="new" value={draft()} attr:value={draft()} onInput={event => {
      ...
      next++;
      setTodos(reconcile(todos.concat(item), {
        key: "id"
      }));
      setDraft("");
      props.onTrace("add", {
        ...
        title: item.title
            ...
            setTodos(produce(storeDraft2 => {
              const alias = storeDraft2.find(item => item.id === todo.id);
              alias.done = checked;
            }));
            ...
            setTodos(reconcile(todos.filter(item => item.id !== todo.id), {
              key: "id"
            }));
