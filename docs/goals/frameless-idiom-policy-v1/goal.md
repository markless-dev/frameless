...
- Existing plan facts (preserve and validate, do not assume correct):
  - Qwik exposes `implicit$FirstArg`; for `$`-suffixed props and APIs the optimizer/runtime accepts a raw function and performs the wrapping. So both `onInput$={$((e, el) => ...)}` and `onInput$={(e, el) => ...}` are valid, with the latter being the idiomatic form.
