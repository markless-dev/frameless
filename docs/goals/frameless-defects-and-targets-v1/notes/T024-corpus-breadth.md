...
`ComponentPropExpression` (`packages/compiler/src/schema.ts:150`) nor
`PropDestructuringEntry` (`packages/compiler/src/schema.ts:205`) carries a **type
field**. So composition emitted from today's IR passes **untyped** props. The
consequence is already visible in shipped output — `packages/frameworks/angular/generated/S2.ts`
...
The Angular emitter documents its rule at `packages/frameworks/angular/src/emitter/index.ts:32`
— "`forVariables` are the enclosing `@for` item names, **OUTERMOST FIRST**" — and
restates it at :475 and :535. Every shipped call site is a one-element list
(`onH7Input(todo, $event)` in `generated/S2.ts`). **A rule whose entire content is an
ordering, exercised only at length one, is folklore.** This scenario is the first
instance of it.
...
**Known risk, flagged rather than hidden:** `packages/frameworks/qwik/src/emitter/index.ts:498`
and `:642` throw `synthesized sync$ body is not closed: unsupported guard condition`.
...
- **S8 last and alone** — the Qwik `sync$` throw at :498/:642 is a live possibility,
  and a batch that may terminate in an emitter finding must not carry a second
...
| --- | --- | --- |
| react | delete `key={todo.id}` from the **inner** `.map(...)` row (shape at `generated/S2.jsx:42`) | inner-row identity survives an outer reorder |
| solid | change the inner `<For each={...}>` to `<Index each={...}>` (`generated/S2.jsx:39`) | `Index` keys by position, not identity |
| qwik | delete `key={...}` from the inner `.map(...)` (`generated/S2.jsx:36`) | resumed reconciliation still keys rows |
| svelte | delete the inner `(todo.id)` from `{#each ... as ... (…)}` (`generated/S2.svelte:27`) | the T009 require-each-key shape, one level deeper |
| vue | delete `:key="todo.id"` from the inner `v-for` (`generated/S2.vue:31`) | same axis, block-based renderer |
