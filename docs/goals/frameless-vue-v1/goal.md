...
- **Official scaffold only, and it must be one that server-renders as it ships.** AMENDED 2026-07-27 by T002. `npm create vue@latest` (create-vue@3.23.0) is a **client-only SPA** — measured by T001: `dev=vite`, entry is `createApp().mount()`, and the entire bundle's only `ssr` hit is an eslint ignore glob. It cannot satisfy this board's own oracle, every clause of which opens with a server-rendered assertion. The scaffold of record is therefore **create-vite-extra's `template-ssr-vue-ts`**, reached from `npm create vite@latest` → Others → create-vite-extra. That is the SAME template family `demos/react-official` and `demos/solid-official` already run: T002 packed create-vite-extra@5.0.2 and diffed `template-ssr-react/server.js` against `demos/react-official/server.js` — they are the same file bar one extracted handler, and the dependency blocks are byte-identical. So the rule the original sentence enforced — **no hand-rolled SSR harness** — is unchanged and is satisfied MORE strongly, not less, because the server ships with the template. T004 must record the provenance chain and a diff against the packed tarball, so "as it ships" stays checkable rather than asserted. The repo has already paid for the lesson that hand-rolled SSR harnesses produce fake failures: see `docs/goals/frameless-qwik-v1/` and the note that the whole "blocked on beta.38" episode was a self-inflicted broken build harness.
...
- ~~`poc/09-storage/vue` took cheap lexical sugar with classic lifecycle shapes.~~ **STRUCK 2026-07-27 by T001.** That directory contains only `build.mjs` and `main.js` — no `package.json`, no `.vue` file, no `vue` dependency. The characterisation is not evidenced by the directory as it stands, so it is removed as an input rather than left standing as a prior.
